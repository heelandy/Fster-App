...
import { AcceptInviteButton } from '@/components/accept-invite';
...
export default async function InvitePage({ searchParams }: { searchParams: { token?: string } }) {
  ...
  const invite = token
    ? await prisma.householdInvite.findUnique({
        ...
        select: { email: true, role: true, acceptedAt: true, expiresAt: true, household: { select: { name: true } } },
      })
    : null;
    ...
    return (
      <Shell>
        ...
        <p className="mt-2 text-sm text-slate-600">
          This invitation is invalid, has already been used, or has expired. Ask whoever invited you to send a new one.
        </p>
  ...
  return (
    <Shell>
      <h1 className="text-xl font-semibold text-slate-900">Join {invite.household.name}</h1>
      <p className="mt-2 text-sm text-slate-600">
        You’ve been invited as a <span className="font-medium">{roleLabel}</span> for the address{' '}
